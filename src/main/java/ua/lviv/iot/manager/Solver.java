...
public class Solver {
    ...
    private static LinkedList<String> track = new LinkedList<>();// для виводу рядка
    // то не обовязково
    ...
    private static Integer check(LinkedList<String> dictionary, String word, Integer depth, LinkedList<String> localTrack) {
        ...
        if (dictionary.contains(word)) {
            ...
            for (int i = 0; i < word.length(); i++) {
                cash = check(dictionary, newWord(word, i), depth, new LinkedList<>(localTrack));
                if (result < cash) {
                    result = cash;
                }
            }
    ...
    public static String newWord(String word, Integer index) {
